...
class TranslateQueueManager {
    ...
    const EXPORT_OPTION = 'salt_translate_queue_export';

    const POSTS_CRON_HOOK = 'salt_translate_posts_event';
    const TERMS_CRON_HOOK = 'salt_translate_terms_event';
    const EXPORT_CRON_HOOK = 'salt_translate_export_event';
    ...
    public function __construct($container) {
        ...
        add_action('wp_ajax_start_export_translation_queue', [$this, 'handle_ajax_start_export_queue']);
    ...
    public function handle_export_queue(): void {
        $this->process_queue('export');
    }
    ...
    private function process_export_queue(): void {
        ...
        $query = new \WP_Query($args);
        if (empty($query->posts)) {
            $this->mark_queue_complete('export');
            return;
        }
        ...
        try {
            ...
            error_log("❌ Post #$post_id export sırasında hata: " . $e->getMessage());
            // hata durumunda işaretlemeyip sonraki cronla tekrar denenmesini sağlayabiliriz.
        }
    }
    ...
    public function get_queue_status(string $type): array {
        ...
        if (($status['status'] ?? '') === 'processing') {
            ...
            $hook = $type === 'export' ? self::EXPORT_CRON_HOOK : $hook;
    ...
    public function handle_ajax_start_export_queue(): void {
        $this->handle_ajax_start_queue('export');
    }
    ...
    public function start_queue(string $lang, string $type) {
        ...
        if ($type === 'post') {
            ...
            update_option(self::EXPORT_OPTION, ['lang' => $lang]);
            wp_clear_scheduled_hook(self::EXPORT_CRON_HOOK);
            wp_schedule_single_event(time() + 5, self::EXPORT_CRON_HOOK);
        }
